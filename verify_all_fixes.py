...
def main():
    ...
    with open('app/services/order_service.py', 'r', encoding='utf-8') as f:
        ...
        if "from flask_login import current_user" in content:
            print("✓ 订单服务已修复（使用current_user）")
            results.append(True)
        else:
            print("✗ 订单服务未修复")
            results.append(False)
        
        if "g.user.is_authenticated" in content:
            print("✗ 订单服务仍使用g.user（需要修复）")
            results.append(False)
        else:
            print("✓ 订单服务不再使用g.user")
            results.append(True)
    
    # 检查通知服务
    print("\n7. 检查通知服务:")
    with open('app/services/notification_service.py', 'r', encoding='utf-8') as f:
        content = f.read()
        if "send_sms_notification_test" in content:
            print("✓ 通知服务方法重名已修复")
            results.append(True)
        else:
            print("✗ 通知服务方法重名未修复")
    ...
    if passed == total:
        ...
        print("4. 访问首页测试订单创建功能")
        print("\n运行测试脚本：")
        print("  python test_order_creation.py")
